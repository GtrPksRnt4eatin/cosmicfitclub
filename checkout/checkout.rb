...
class Checkout < Sinatra::Base
  ...
  post('/save_card',        :self => params[:customer_id] ) { save_card           }
  post('/set_default_card', :self => params[:customer_id] ) { set_default_card    }
  post('/remove_card'),     :self => params[:customer_id] ) { remove_card         }
