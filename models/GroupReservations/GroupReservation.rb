class GroupReservation < Sequel::Model
  ...
  def publish_gcal_event
    if self.gcal_event_id then
      ...
      event_id = Calendar::create_rental(start_time, end_time, customer_string, resource.gcal_resource_id)
  ...
  def send_confirmation_emails
    ...
    slots.each do |s|
      ...
      Mail.point_reservation(s.customer.email, model) if self.resource.id == 1
      Mail.floor_reservation(s.customer.email, model) if self.resource.id == 2  
    end
  end
  ...
  def to_public_daypilot(logged_in=nil)
    ...
    { :start => self.start_time.strftime("%Y/%m/%dT%H:%M:%S"),
      ...
      :resource => self.resource ? self.resource.gcal_resource : "Loft-1F-Front (4)"
    }
  end
