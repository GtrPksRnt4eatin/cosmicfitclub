class ClassDefRoutes < Sinatra::Base
  ...
  post '/generate' do
    ...
    Calendar::get_day_events(day.to_s).each do |evt|
      res = GroupReservation.where( :gcal_event_id => evt[:id] ).first
  ...
  post '/reservation' do
    ...
    when "teacher_pass"
      ...
      occurrence.teacher.customer.use_class_pass(msg) { reservation = occurrence.make_reservation( params[:customer_id] )} or halt 400
    when "class_pass"
