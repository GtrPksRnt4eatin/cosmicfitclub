...
def Staff::payroll_csv(from,to)
  ...
  proll.each do |teacher_row|
    ...
    teacher_row[:class_occurrences].each do |row|
      ...
      totals.merge!( { :headcount => row[:headcount], :passes => row[:passes_total], :memberships => row[:membership_total], :payments => row[:payment_total] } ) { |k,v1,v2| v1 + v2.nil? ? 0 : v2 }
    end
    grand_totals.merge!( totals ) { |k,v1,v2| v1 + v2.nil? ? 0 : v2 }
    csv << [ ]
    csv << [ '','TOTALS', totals[:headcount], totals[:passes], totals[:memberships], "$ %.2f" % totals[:payments] ]
    csv << []
  end
  csv << []
  csv << [ '','GRAND TOTALS', grand_totals[:headcount], grand_totals[:passes], grand_totals[:memberships], "$ %.2f" % grand_totals[:payments] ]
  csv.rewind
  csv
end