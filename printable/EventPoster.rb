...
module EventPoster
  ...
  def EventPoster::generate_for_bot(event)
    arr = []
    arr.push({ :img => EventPoster::generate4x5(event.id),       :title => "#{event.title.truncate(12)}_fbpost"  })
    arr.push({ :img => EventPoster::generate1080x1080(event.id), :title => "#{event.title.truncate(12)}_square"  })
    arr.push({ :img => EventPoster::generate1080x1920(event.id), :title => "#{event.title.truncate(12)}_story"   })
    arr.push({ :img => EventPoster::generateFBEvent(event.id),   :title => "#{event.title.truncate(12)}_fbevent" })
  end
